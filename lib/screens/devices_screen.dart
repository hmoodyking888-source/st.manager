...
class _DevicesScreenState extends State<DevicesScreen> with WidgetsBindingObserver {
  ...
  /// إدارة حالة التطبيق لإيقاف المؤقت عند الخروج وتخفيف الضغط
  @override
  ...
  /// بدء التحديث التلقائي كل 30 ثانية لتقليل الضغط على الميكروتك
  ...
  Future<void> _loadDevices() async {
    ...
    try {
      ...
      for (final reg in registrations) {
        ...
        // إصلاح الخطأ البرمجي هنا باستخدام الأقواس لتجنب تعارض المعاملات
        final name = reg['comment']?.toString() ?? 
                     reg['radio-name']?.toString() ?? 
                     (mac.isNotEmpty ? mac : 'Station');
  ...
  /// تطبيق البحث مع تحسين الأداء (Debounce)
  void _applySearch() {
...
'''
  ...
  Future<void> _saveNetwatchDevice({
    ...
    final identityResult = await widget.routerService?.sendCommand('/system/identity/print');
    ...
    if (telegramEnabled && botToken.trim().isNotEmpty && chatId.trim().isNotEmpty) {
    ...
    if (isNew) {
      await widget.routerService?.sendCommand('/tool/netwatch/add', params: params);
    } else {
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                TextField(
                  controller: nameCtrl,
                  decoration: const InputDecoration(labelText: 'الاسم / التعليق'),
                ),
                TextField(
                  controller: ipCtrl,
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'المنفذ / الواجهة'),
                  textDirection: TextDirection.ltr,
                ),
                TextField(
                ...
                TextField(
                  controller: userCtrl,
                  decoration: const InputDecoration(labelText: 'اسم المستخدم (اختياري)'),
                ),
                TextField(
                  controller: passCtrl,
  ...
  String _translateSource(String source) {
    switch (source) {
      case 'wireless': return 'جدول التسجيل اللاسلكي';
      case 'wireless-interface': return 'واجهة لاسلكية';
      case 'dhcp': return 'خادم DHCP';
      case 'netwatch': return 'مراقبة Netwatch';
      case 'neighbor': return 'اكتشاف الجيران';
      ...
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                Icon(Icons.devices_other, color: onSurface.withValues(alpha: 0.6)),
    ...
    if (_errorMessage != null && _devices.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.error_outline, size: 48, color: AppTheme.redOffline.withValues(alpha: 0.7)),
    ...
    if (_filteredDevices.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.devices_other, size: 64, color: onSurface.withValues(alpha: 0.2)),
            const SizedBox(height: 16),
            Text(
              _searchQuery.isEmpty ? 'لا توجد أجهزة متصلة' : 'لا توجد نتائج مطابقة',
              style: TextStyle(color: onSurface.withValues(alpha: 0.5), fontSize: 16),
            ),
            if (_searchQuery.isEmpty) ...[
              const SizedBox(height: 16),
              ElevatedButton.icon(
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          final model = d['deviceModel']?.toString() ?? 'Unknown';
          final type = d['type']?.toString() ?? 'Unknown';
          
          return _DeviceCard(
            key: ValueKey(d['id']?.toString() ?? i),
            device: d,
            onSurface: onSurface,
            iconColor: _getDeviceColor(model, type),
            iconData: _getDeviceIcon(model, type),
            statusColor: _statusColor(d['status'] ?? 'unknown'),
            statusText: _statusText(d['status'] ?? 'unknown'),
            sourceTranslated: _translateSource(d['source']?.toString() ?? ''),
...
class _DeviceCard extends StatelessWidget {
  ...
  const _DeviceCard({
    ...
    required this.sourceTranslated,
        ...
        subtitle: Row(
          children: [
            Container(
              ...
              child: Text(
                model,
                style: TextStyle(color: iconColor.withValues(alpha: 0.9), fontSize: 10, fontWeight: FontWeight.w600),
              ),
            ),
            ...
            if (signal.isNotEmpty)
              Tooltip(
                ...
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(Icons.signal_cellular_alt, size: 16, color: statusColor),
                    const SizedBox(width: 2),
                    Text(signal, style: TextStyle(color: statusColor, fontSize: 11, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            const SizedBox(width: 8),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (ip.isNotEmpty) _DetailRow(label: 'عنوان IP', value: ip, isLtr: true),
                if (mac.isNotEmpty) _DetailRow(label: 'عنوان MAC', value: mac, isLtr: true),
                if (uptime.isNotEmpty) _DetailRow(label: 'مدة التشغيل', value: uptime, isLtr: true),
                if (ssid.isNotEmpty) _DetailRow(label: 'SSID', value: ssid),
                if (frequency.isNotEmpty) _DetailRow(label: 'التردد', value: frequency, isLtr: true),
                if (interface.isNotEmpty) _DetailRow(label: 'الواجهة', value: interface, isLtr: true),
                if (rxRate.isNotEmpty) _DetailRow(label: 'سرعة الاستقبال', value: rxRate, isLtr: true),
                if (txRate.isNotEmpty) _DetailRow(label: 'سرعة الإرسال', value: txRate, isLtr: true),
                if (lastActivity.isNotEmpty) _DetailRow(label: 'آخر نشاط', value: lastActivity, isLtr: true),
          ...
          Expanded(
            child: Directionality(
              ...
              child: Text(
                ...
                textAlign: isLtr ? TextAlign.right : TextAlign.start,
              ),
            ),
          ),
        ],
