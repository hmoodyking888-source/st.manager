      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          BottomNavigationBarItem(
              icon: Icon(Icons.router), label: 'أجهزة الشبكة'),
          BottomNavigationBarItem(
              icon: Icon(Icons.monitor_heart), label: 'مراقبة Netwatch'),
        ],
      ),
    );
  }
...
class _NeighborsTabState extends State<NeighborsTab> {
  // متغيرات التخزين المؤقت (Static) للحفاظ على البيانات عند التبديل بين الصفحات
  ...
  void initState() {
    super.initState();
    _fetchNetworkData(forceRefresh: false); // استخدام الكاش إذا كان متاحاً
  }
  ...
  Future<void> _fetchNetworkData({bool forceRefresh = true}) async {
    // التحقق من وجود بيانات مخزنة مسبقاً لتخفيف الضغط على الميكروتك
    if (!forceRefresh && _cachedDevices != null && _cachedDevices!.isNotEmpty) {
      if (mounted) {
        setState(() {
          _devices = _cachedDevices!;
          _availableInterfaces = _cachedInterfaces ?? ['الكل'];
    ...
    try {
      ...
      try {
        final resN =
            await widget.routerService!.sendCommand('/ip/neighbor/print');
      ...
      for (var n in neighbors) {
        ...
        final interface =
            _formatInterface(item['interface']?.toString() ?? 'غير معروف');
      ...
      _cachedDevices = loadedDevices;
      _cachedInterfaces = interfacesSet.toList();

      if (mounted) {
        setState(() {
          _devices = loadedDevices;
          _availableInterfaces = _cachedInterfaces!;
          _filteredDevices = _devices.where(_matchesFilters).toList();
          _isLoading = false;
        });
      }
  ...
  bool _matchesFilters(NetworkDevice d) {
    final matchesType =
        _selectedTypeFilter == 'الكل' || d.type == _selectedTypeFilter;
    final matchesInterface = _selectedInterfaceFilter == 'الكل' ||
        d.interface == _selectedInterfaceFilter;
  ...
  Future<void> _openInBrowser(String ip, {bool inApp = false}) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('تعذر فتح المتصفح للآيبي: $ip',
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildBottomSheetButton('فتح في المتصفح', () {
                Navigator.pop(context);
                _openInBrowser(device.ip, inApp: false);
              }),
              _buildBottomSheetButton('إعادة التشغيل', () {
                Navigator.pop(context);
                _showRebootConfirmation(device);
              }),
              _buildBottomSheetButton('فتح في التطبيق', () {
                Navigator.pop(context);
                _openInBrowser(device.ip, inApp: true);
              }),
              _buildBottomSheetButton('تفاصيل أكثر', () {
                Navigator.pop(context);
  ...
  Widget _buildBottomSheetButton(String title, VoidCallback onTap) {
    return Container(
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text(
                        'تم إرسال أمر إعادة التشغيل إلى ${device.name}',
                        style: const TextStyle(fontFamily: 'Cairo')),
  ...
  void _showMoreDetails(NetworkDevice device) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: _cardColor,
        title:
            Text('تفاصيل ${device.name}', style: TextStyle(color: _goldColor)),
        content: SizedBox(
        ...
        actions: [
          IconButton(
              ...
              onPressed: _isLoading
                  ? null
                  : () => _fetchNetworkData(forceRefresh: true),
                ...
                return Padding(
                  ...
                  child: ChoiceChip(
                    ...
                    shape: RoundedRectangleBorder(
                        ...
                        side: BorderSide(
                            color:
                                isSelected ? _goldColor : Colors.transparent)),
                  ),
                );
              }).toList(),
            ),
              ...
              children: _availableInterfaces.map((filter) {
                ...
                return Padding(
                  ...
                  child: ChoiceChip(
                    ...
                    backgroundColor: _bgColor,
                    labelStyle: TextStyle(
                        color: isSelected ? Colors.blueAccent : _subTextColor,
                        fontWeight:
                            isSelected ? FontWeight.bold : FontWeight.normal),
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(8),
  ...
  Widget _buildDeviceList() {
    ...
    if (_errorMessage != null) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.wifi_off,
                size: 64, color: Colors.redAccent.withValues(alpha: 0.8)),
            const SizedBox(height: 16),
            Text(_errorMessage!, style: TextStyle(color: _textColor)),
            ElevatedButton(
              style: ElevatedButton.styleFrom(
                  backgroundColor: _goldColor, foregroundColor: _bgColor),
              onPressed: () => _fetchNetworkData(forceRefresh: true),
    ...
    if (_filteredDevices.isEmpty)
      return Center(
          child: Text('لا توجد أجهزة مطابقة للبحث',
              style: TextStyle(color: _subTextColor, fontSize: 16)));

    return RefreshIndicator(
      color: _goldColor,
      backgroundColor: _cardColor,
      onRefresh: () => _fetchNetworkData(forceRefresh: true),
      child: ListView.builder(
                ...
                Row(
                  ...
                  children: [
                    Container(
                      ...
                      child: Icon(_getTypeIcon(device.type),
                          color: _goldColor, size: 28),
                    ),
                  ...
                  children: [
                    Expanded(
                        child: _buildInfoColumn(
                            Icons.memory, 'الموديل', device.model)),
                    Expanded(
                        child: _buildInfoColumn(Icons.settings_ethernet,
                            'المدخل', device.interface)),
                  ],
                ),
                const SizedBox(height: 16),
                Row(
                  children: [
                    Expanded(
                        child: _buildInfoColumn(
                            Icons.network_check, 'الآيبي (IP)', device.ip,
                            isLtr: true)),
                    Expanded(
                        child: _buildInfoColumn(
                            Icons.fingerprint, 'الماك (MAC)', device.mac,
                            isLtr: true)),
                  ],
                ),
              ],
...
class _NetwatchTabState extends State<NetwatchTab> {
  ...
  bool _isValidIpv4(String ip) {
    final value = ip.trim();
    if (value.isEmpty || value == 'لا يوجد IP' || value == 'غير معروف')
      return false;
  ...
  String _automaticName(Map<String, dynamic> device, int index) {
    ...
    if (identity.isNotEmpty && identity.toLowerCase() != 'unknown')
      return identity;
  ...
  String _scriptForTelegram(
      {required String token, required String chat, required String message}) {
  ...
  Future<void> _syncAutomaticDevicesFromTelegram() async {
    ...
    try {
      ...
      final neighbors =
          neighborsResponse is List ? neighborsResponse : <dynamic>[];
      ...
      for (final raw in neighbors) {
        ...
        final old = mac.isNotEmpty
            ? byMac[mac.toLowerCase()]
            : byHost[ip.toLowerCase()];
        final name = old == null
            ? newName
            : _displayComment(old['comment']?.toString() ?? '', ip);

        final upScript = notifyUp
            ? _scriptForTelegram(
                token: token,
                chat: chat,
                message: '✅ القطعة $name ($ip) عادت إلى العمل.')
            : '';
        final downScript = notifyDown
            ? _scriptForTelegram(
                token: token,
                chat: chat,
                message: '❌ القطعة $name ($ip) توقفت عن العمل.')
            : '';
          ...
          if (id.isNotEmpty) {
            await router.sendCommand('/tool/netwatch/set', params: {
              ...
              'comment': _buildTelegramComment(
                  ...
                  mac.isNotEmpty
                      ? mac
                      : _extractMac(old['comment']?.toString() ?? '')),
              'up-script': upScript,
  ...
  Future<void> _fetchNetwatchData({bool silent = false}) async {
    ...
    try {
      try {
        final clockRes =
            await widget.routerService!.sendCommand('/system/clock/print');
      ...
      if (res is List) {
        for (final raw in res) {
          ...
          if (previousStatus != normalizedStatus ||
              !_localStatusSince.containsKey(statusKey)) {
            if (sinceTime == null) {
              _localStatusSince[statusKey] =
                  DateTime.now().subtract(_timeOffset);
            } else {
    ...
    try {
      ...
      final upScript = (token.isNotEmpty && chat.isNotEmpty && notifyUp)
          ? _scriptForTelegram(
              token: token,
              chat: chat,
              message: '✅ القطعة $cleanName ($cleanIp) عادت إلى العمل.')
          : '';
      final downScript = (token.isNotEmpty && chat.isNotEmpty && notifyDown)
          ? _scriptForTelegram(
              token: token,
              chat: chat,
              message: '❌ القطعة $cleanName ($cleanIp) توقفت عن العمل.')
          : '';
      ...
      for (final raw in entries) {
        if (raw is! Map) continue;
        final item = Map<String, dynamic>.from(raw);
        if ((item['host']?.toString().trim().toLowerCase() ?? '') ==
            cleanIp.toLowerCase()) {
          existing = item;
          break;
        }
      ...
      if (existing != null && existing['.id'] != null) {
        await widget.routerService!.sendCommand('/tool/netwatch/set', params: {
          ...
          'comment': existing['comment']
                      ?.toString()
                      .startsWith(_telegramCommentPrefix) ==
                  true
              ? _buildTelegramComment(
                  cleanName, _extractMac(existing['comment']?.toString() ?? ''))
              : cleanName,
          'up-script': upScript,
          'down-script': downScript,
        });
        ...
        await widget.routerService!.sendCommand('/tool/netwatch/add', params: {
          'host': cleanIp,
          'comment': token.isNotEmpty && chat.isNotEmpty
              ? _buildTelegramComment(cleanName, '')
              : cleanName,
          'up-script': upScript,
          'down-script': downScript,
        });
  ...
  Future<void> _updateDevice(
      NetwatchDevice device, String name, String ip) async {
    if (widget.routerService == null) return;
    ...
    try {
      ...
      if (managed || (token.isNotEmpty && chat.isNotEmpty)) {
        params['up-script'] = (token.isNotEmpty && chat.isNotEmpty && notifyUp)
            ...
            : '';
        params['down-script'] =
            (token.isNotEmpty && chat.isNotEmpty && notifyDown)
                ? _scriptForTelegram(
                    token: token,
                    chat: chat,
                    message: '❌ القطعة $cleanName ($cleanIp) توقفت عن العمل.')
  ...
  void _showDeviceSettingsDialog({NetwatchDevice? device}) async {
    ...
    String name =
        device != null ? _displayComment(device.comment, device.host) : '';
    String user = '';
    String pass = '';
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Align(
                alignment: Alignment.centerRight,
                child: Text('اسم المستخدم مثل ubnt',
                    style: TextStyle(color: Colors.white70, fontSize: 13)),
              ),
              const SizedBox(height: 4),
              TextField(
              ...
              const Text('اضغط فحص للتأكد أن كلمة المرور صحيحة',
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            const Color(0xFFC46CFF), // لون زر فحص مطابق للصورة
                        shape: RoundedRectangleBorder(
                      ...
                      onPressed: () {
                        _showSnack(
                            'جاري فحص الاتصال بالقطعة...', Colors.orange);
                        Future.delayed(const Duration(seconds: 1), () {
                          if (mounted)
                            _showSnack('تم فحص الاتصال بنجاح', Colors.green);
                        });
                      },
                      child: Text('فحص',
                          style: TextStyle(
                  ...
                  Expanded(
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            const Color(0xFFC46CFF), // لون زر حفظ مطابق للصورة
                        shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(4)),
                        padding: const EdgeInsets.symmetric(vertical: 12),
                      ),
  ...
  void _rebootDeviceCommand(NetwatchDevice device) {
    // هنا يتم وضع الكود البرمجي الفعلي لإرسال أمر إعادة التشغيل إن وجد في الـ API الخاص بك
    // سيتم إظهار رسالة تفيد بنجاح إرسال الأمر كإجراء شكلي حالياً
    _showSnack('تم إرسال أمر إعادة التشغيل إلى القطعة بنجاح', Colors.green);
  }
  ...
  void _showDeviceActions(NetwatchDevice device) {
    ...
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent, // شفاف لإظهار الزوايا الدائرية
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              ...
              _buildBottomSheetButton('فتح في المتصفح', () {
                Navigator.pop(ctx);
                _openInBrowser(device.host, inApp: false);
              }),
              _buildBottomSheetButton('إعادة التشغيل', () {
                Navigator.pop(ctx);
                _rebootDeviceCommand(device);
              }),
              _buildBottomSheetButton('فتح في التطبيق', () {
                Navigator.pop(ctx);
                _openInBrowser(device.host, inApp: true);
              }),
              _buildBottomSheetButton('تعديل', () {
                Navigator.pop(ctx);
                _showDeviceSettingsDialog(device: device);
              }),
              _buildBottomSheetButton('حذف', () {
                Navigator.pop(ctx);
  ...
  // دالة مساعدة لإنشاء أزرار القائمة السفلية لتتطابق مع الصورة (أزرار زرقاء/بنفسجية)
  Widget _buildBottomSheetButton(String title, VoidCallback onTap) {
    return Container(
      width: double.infinity,
      margin: const EdgeInsets.only(bottom: 8),
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          backgroundColor: const Color(0xFF7B66FF), // لون مقارب للصورة المرفقة
          shape:
              RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          padding: const EdgeInsets.symmetric(vertical: 14),
        ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _bgColor,
      // تمت إزالة الشريط العلوي تماماً، وإضافة زر عائم للإضافة
      floatingActionButton: FloatingActionButton(
        backgroundColor: const Color(0xFF7B66FF),
        onPressed: () => _showDeviceSettingsDialog(),
        child: const Icon(Icons.add, color: Colors.white),
      ),
      body: _buildNetwatchList(),
    );
  }
          ...
          children: [
            ...
            ElevatedButton(
              ...
              child: Text('إعادة المحاولة',
                  style:
                      TextStyle(color: _bgColor, fontWeight: FontWeight.bold)),
            ),
          ],
        ),
      );
      ...
      return RefreshIndicator(
        ...
        child: ListView(
          ...
          children: const [
            SizedBox(height: 120),
            Center(
                child: Text('لا توجد قطع في Netwatch',
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        padding: const EdgeInsets.only(
            top: 40, bottom: 80), // إضافة بادينج من الأعلى والأسفل
        physics: const AlwaysScrollableScrollPhysics(),
        ...
        itemBuilder: (context, index) {
          ...
          return Container(
            ...
            child: Row(
              children: [
                // زر الإجراءات على اليسار (سهم لأسفل)
                InkWell(
                ...
                // بيانات القطعة (الاسم، الآيبي، الإشارة)
                Expanded(
                  flex: 6,
                  child: Column(
                ...
                Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      (device.comment.toLowerCase().contains('rocket') ||
                              device.comment.toLowerCase().contains('omni'))
                          ? 'مرسل'
                          : 'مستقبل',
                      style:
